...
%% Examples
ex = cell(2,2,4); % rows: (1) bouton, (2) neuron, columns: (1) ori, (2) dir
ex(1,1,:) = {'SS076', '2017-10-02', 6, 4};
ex(1,2,:) = {'SS076', '2017-10-02', 6, 6};
ex(2,1,:) = {'SS044', '2015-04-28', 2, 44};
ex(2,2,:) = {'SS044', '2015-04-28', 2, 7};
...
for k = 1:2
    str = sets{k};
    for j = 1:size(ex,2)
        f = fullfile(folders.data, str, ex{k,j,1}, ex{k,j,2});
        calc = io.getCalciumData(f);
        stim = io.getGratingInfo(f, 'drifting');

        planes = readNPY(fullfile(folders.data, 'boutons', ex{k,j}, ...
