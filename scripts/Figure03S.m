function Figure03S(folders)
    ...
    for s = 1:2 % boutons and neurons
        ...
        % % Plot RF distance to monitor edge vs tuning preference for each
        % % dataset
        % figure('Position', [40 580 1850 410])
        % tiledlayout(1, 4)
        % xMax = 0;
        ...
        %             tn = tuned{k,feat} < maxP;
        %             if sum(tn) < minUnits
        %                 continue
        %             end
        %             pr = prefs{k,feat}(tn);
        %             d = rfDist{k}(tn,ed);
        %             [x, y] = smoothCircular(d, pr, types{feat}, ...
        %                 smoothN(2), smoothPars(2));
        %             if any(y < -30/feat) || any(y > 390/feat)
        %                 y = mod(y, 360/feat);
        %                 ind = find(abs(diff(y)) > 180/feat)';
        %                 for n = flip(ind)
        ...
        %         xlim([0 xMax])
        %         ylim([-30 390]./feat)
        %         set(gca, "Box", "off", "YTick", (-30:90:390)./feat, ...
        %             "ColorOrder", turbo(size(prefs,1)))
        %         xlabel(sprintf('Distance to %s monitor edge (deg)', labels{1,ed}))
        %         ylabel(sprintf('Preferred %s (deg)', labels{2,feat}))
        %     end
        % end
        % sgtitle(sets{s})
        % io.saveFigure(gcf, fPlots, ...
        %     sprintf('rf-to-monitorEdge_%s_perDataset', sets{s}))
    ...
    % function [x, y] = smoothCircular(distances, angles, type, numSamples, par)
    %     switch type
    %         case 'dir'
    %             scalar = 1;
    %         case 'ori'
    ...
    %     end
    % 
    %     [dist_sorted, order_] = sort(distances, "ascend");
    %     angl_sorted = angles(order_);
    %     dist_sm = smoothdata(dist_sorted, "movmean", numSamples);
    %     dist_sm = dist_sm(ceil(numSamples/2) : end-floor(numSamples/2));
    %     angl_sm  = NaN(length(distances) - numSamples + 1, 1);
    %     for j = 1:length(angl_sm)
    %         angl_sm(j) = tuning.vectorAveraging(ones(1,numSamples), ...
    %             angl_sorted(j:j+numSamples-1) .* scalar) ./ scalar;
    %     end
    %     if ~isnan(par)
    %         angl_sm = tuning.continuousAngles(angl_sm, type);
    %         f1_ = fit(dist_sm, angl_sm, "smoothingspline", ...
    %             "SmoothingParam", par);
    %         x = (floor(dist_sm(1)*10)/10 : 0.1 : ceil(dist_sm(end)*10)/10)';
    %         y = f1_(x);
    %     else
    %         x = dist_sm;
    %         y = angl_sm;
