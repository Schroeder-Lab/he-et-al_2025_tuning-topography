...
%ALIGNDATA   Aligns times of some events (e.g. spike times) to other events 
%(e.g. stimulus presentation times). 
%Events that will be aligned are called ev1.
%Events that are the "anchors" are called ev2.
%
% INPUTS
% dataTimes         [ev1]; the absolute time of each ev1
% alignTimes        [ev2]; the absolute time of each ev2
% limits            [pre post]; relative time spans before and after each
%                   ev2, in which aligned ev1 can fall

% OUTPUTS
% times_aligned     [ev1_al]; the time of each ev1 relative to any ev2
%                   +-limits
