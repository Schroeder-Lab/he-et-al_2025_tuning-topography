...
% for ephys data, also load information on other stimulus features
if strcmp(type, 'gratingsDrifting') && ...
        isfile(fullfile(folder, '_ss_gratingsDriftingID.contrasts.npy'))
    data.contrasts = double(readNPY(fullfile(folder, ...
        '_ss_gratingsDriftingID.contrasts.npy')));
    data.spatialFrequencies = double(readNPY(fullfile(folder, ...
        '_ss_gratingsDriftingID.spatialFrequencies.npy')));
    data.temporalFrequencies = double(readNPY(fullfile(folder, ...
        '_ss_gratingsDriftingID.temporalFrequencies.npy')));
end